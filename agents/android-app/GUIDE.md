...
- **普通模式不做前台应用检测**：Android 非 root 下无法可靠获取前台应用。normal 模式只上报在线、电量和健康数据。
- **root / LSPosed 高级模式**：root 模式低频读取系统 dumpsys；LSPosed 模式固定 system scope，优先使用系统服务事件，避免 hook 全部应用。
